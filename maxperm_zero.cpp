...
ofstream out_perm("out_perm.txt");
...
double Perm(igraph_t * g, igraph_vector_t *neig_v, int * vertex_community, int v)
{
	...
	free(E_v);
	free (edge_temp);
	//out_perm<<"i: "<<v<<" "<<I_v<<" "<<Emax_v<<" "<<D_v<<" "<<cin_v<<" "<<((1.0*I_v + 1)/(Emax_v*(D_v + 1)))-1+cin_v<<endl;
	return ((1.0*I_v + 1)/(Emax_v*(D_v + 1)))-1+cin_v;	
}
...
double igraph_community_MaxPerm(igraph_t * g, igraph_vector_t * membership, int maxIt)
{
	...
	while(Sum>Old_Sum && Itern<maxIt)
	{
		...
		for (i = 0; i < vertices; ++i)
		{
			out_perm<<"tt"<<" "<<i + 1<<" "<<vertex_community[i]<<endl;
		}
		for (i = 0; i < vertices; i++)
		{
			double p =  Perm(g, &(neig[i]), vertex_community, i); 
			Sum += p;
			out_perm<<"ee"<<" "<<p<<endl;
		}
